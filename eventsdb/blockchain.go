...
func connectWithRetry(rpcURL string, maxRetries int, retryDelay time.Duration) (*ethclient.Client, error) {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if err != nil {
			fmt.Printf("Dial failed on attempt %d: %v\n", i+1, err)
			if i < maxRetries-1 {
		...
		fmt.Printf("Connection established, testing with HeaderByNumber...\n")
		...
		if header == nil {
			fmt.Printf("Connection test returned nil header on attempt %d\n", i+1)
			client.Close()
		...
		fmt.Printf("Successfully connected to RPC endpoint on attempt %d!\n", i+1)
